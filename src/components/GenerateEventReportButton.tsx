// components/GenerateEventReportButton.tsx
'use client';

import { generateEventsReport } from "@/lib/actions";
import { useState } from "react";
import Image from "next/image";

export default function GenerateEventReportButton() {
  const [isLoading, setIsLoading] = useState(false);

  const handleGenerateReport = async () => {
    setIsLoading(true);
    try {
      const pdfDataUri = await generateEventsReport();
      ...
      a.href = url;
      a.download = `events-report-${new Date().toISOString().slice(0, 10)}.pdf`;
